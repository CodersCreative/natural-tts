...
use candle_transformers::{generation::LogitsProcessor, models::{encodec, metavoice::{adapters, gpt, transformer}}};
...
impl MetaModel{
    ...
    pub fn generate (&mut self, prompt : String) -> Result<SynthesizedAudio<f32>, Box<dyn Error>>{
        ...
        let encodec_vb = unsafe { VarBuilder::from_mmaped_safetensors(&[self.encodec_weights.clone()], self.dtype, &encodec_device)? };
        ...
        for index in 0..self.max_tokens {
            ...
            println!("gen");
            let logits = self.first_stage_model.forward(&input, &spk_emb, tokens.len() - context_size)?;
        ...
        let audio_ids = Tensor::new(audio_ids, &encodec_device).unwrap().unsqueeze(0).unwrap();
